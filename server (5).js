...
function getLobbyPlayerList(lobbyCode) {
  ...
  lobby.players.forEach((p, id) => {
    list.push({ id, name: p.name, color: p.color, x: p.x, y: p.y, z: p.z, rx: p.rx, ry: p.ry, hp: p.hp ?? 100 });
  });
  return list;
}
  ...
  ws.on("message", (raw) => {
    ...
    switch (msg.type) {
      ...
      case "join_lobby": {
        ...
        broadcast(code, {
          type: "player_joined",
          player: { id, name: client.name, color: client.color, x: 0, y: 1, z: 0, rx: 0, ry: 0, hp: 100 },
        }, ws);
      ...
      case "attack": {
        ...
        const damage = Math.min(50, Math.max(0, parseInt(msg.damage) || 0)); // clamp 0-50
        const weapon = (msg.weapon || "Fist").substring(0, 20);

        targetPlayer.hp = Math.max(0, (targetPlayer.hp ?? 100) - damage);

        // Tell the target they took damage
        clients.forEach((c, cws) => {
          if (c.id === targetId && cws.readyState === 1) {
...
server.listen(PORT, "0.0.0.0", () => console.log(`🐒 Gorilla Tag Server running on port ${PORT}`));
